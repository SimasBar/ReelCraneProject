...
int main(int argc, char *argv[]) {
  ...
  int last_emitted = -1;  // track last emitted value to prevent resending same signal
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    if (timer > 0 && wb_robot_get_time() >= timer) {
      ...
      if (last_emitted != 1) {
        const char *signal = "1";
        wb_emitter_send(belt_emitter, signal, strlen(signal));
        printf("Emitted (timer stop): %s\n", signal);
        last_emitted = 1;
      }
      break;
    }
